...
def vind_afbeelding(entry):
    """Zoekt een afbeelding in het feed-item. Niet elke bron heeft er een."""
    for sleutel in ("media_content", "media_thumbnail"):
        media = entry.get(sleutel)
        if media and isinstance(media, list):
            url = media[0].get("url")
            if url:
                return url
    for bijlage in entry.get("enclosures", []) or []:
        if "image" in (bijlage.get("type") or ""):
            return bijlage.get("href")
    ruw = entry.get("summary", "") or entry.get("description", "")
    m = re.search(r'<img[^>]+src=["\']([^"\']+)["\']', ruw)
...
def stuur_telegram(token: str, chat_id: str, tekst: str,
                   afbeelding: str = None) -> bool:
    """
    Met afbeelding: stuurt een foto met het bericht eronder.
    Zonder: stuurt tekst en laat Telegram zelf een voorbeeldkaartje
    van het artikel ophalen - daar zit meestal ook een foto in.
    """
    if afbeelding:
        try:
            resp = requests.post(
                f"https://api.telegram.org/bot{token}/sendPhoto",
                json={"chat_id": chat_id, "photo": afbeelding,
                      "caption": tekst[:1000], "parse_mode": "HTML"},
                timeout=20,
            )
            if resp.status_code == 200:
                return True
            print(f"[telegram] foto mislukt, val terug op tekst: "
                  f"{resp.status_code}")
        except Exception as exc:  # noqa: BLE001
            print(f"[telegram] foto fout: {exc}")
...
def main() -> int:
    ...
    for naam, url in bronnen:
        ...
        for item in items:
            ...
            treffers.append({
                ...
                "afbeelding": vind_afbeelding(item),
            })
        time.sleep(1)
    ...
    for hit in treffers[:MAX_ALERTS_PER_RUN]:
        ...
        if dry_run:
            ...
            stuur_telegram(token, chat_id, bericht, hit.get("afbeelding"))
            time.sleep(0.5)
